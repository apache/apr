...
static void test_dbd_sqlite2(abts_case *tc, void *data)
{
    ...
    ABTS_ASSERT(tc, apr_psprintf(p, "failed to fetch sqlite2 driver: %pm", &rv), rv == APR_SUCCESS);
    ...
    ABTS_ASSERT(tc, apr_psprintf(p, "failed to open sqlite2 database: %pm", &rv), rv == APR_SUCCESS);
